...
from database.connections_mdb import active_connection, all_connections, delete_connection, if_active, make_active, \
    make_inactive
from info import ADMINS, AUTH_CHANNEL, AUTH_USERS, SUPPORT_CHAT_ID, ON_WATCH, ON_DWNLD, F2LINK_C, CUSTOM_FILE_CAPTION, MSG_ALRT, PICS, AUTH_GROUPS, P_TTI_SHOW_OFF, GRP_LNK, CHNL_LNK, NOR_IMG, LOG_CHANNEL, SPELL_IMG, MAX_B_TN, IMDB, \
...
from utils import get_size, is_subscribed, get_hash, get_poster, search_gagala, temp, get_settings, save_group_settings, get_shortlink, get_tutorial, send_all, get_cap
    ...
    await query.edit_message_reply_markup(
        ...
        [
            [
                ...
                InlineKeyboardButton("✨ ᴜᴘᴅᴀᴛᴇ ᴄʜᴀɴɴᴇʟ ✨", url=GRP_LNK)
             ],[
    ...
    if gl == False:
        ...
        if k == False:
            ...
            if files:
                ...
                await auto_filter(bot, query, k, edit_message=query.message, movie_name=movie)
            else:
...
async def qualities_cb_handler(client: Client, query: CallbackQuery):
    ...
    btn.insert(
        ...
        [
            InlineKeyboardButton(
                text="👇 sᴇʟᴇᴄᴛ ʏᴏᴜʀ ǫᴜᴀʟɪᴛʏ 👇", callback_data="ident"
            )
        ],
    )
    req = query.from_user.id
    ...
    if settings["button"]:
        ...
        btn.insert(0,
            [
                InlineKeyboardButton("ǫᴜᴀʟɪᴛɪᴇs", callback_data=f"qualities#{key}"), 
                InlineKeyboardButton("ʟᴀɴɢᴜᴀɢᴇs", callback_data=f"languages#{key}"),
                InlineKeyboardButton("sᴇᴀꜱᴏɴs",  callback_data=f"seasons#{key}"),
            ]
            )
        btn.insert(0, [
        InlineKeyboardButton("ꜱᴇɴᴅ ᴀʟʟ", callback_data=f"sendfiles#{key}")
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        btn2 = [[
                 ...
                 InlineKeyboardButton("Rᴇᴏ̨ᴜᴇsᴛ Gʀᴏᴜᴘ Lɪɴᴋ", url="https://t.me/Bishnoi_botz_support")
               ]]
        if query.from_user.id in ADMINS:
            user = await client.get_users(from_user)
            reply_markup = InlineKeyboardMarkup(btn)
        ...
        buttons = [[
            InlineKeyboardButton('ᴄᴏɴᴛᴀᴄᴛ sᴜᴘᴘᴏʀᴛ', url='https://t.me/J_shree_ram')
        ],[
            InlineKeyboardButton('⇍ ʙᴀᴄᴋ', callback_data='start'),
            InlineKeyboardButton('ᴄʟᴏsᴇ ⊝', callback_data='close_data')
        ]]
        ...
        await query.message.edit_text(
            ...
            btn = [[
                    ...
                    InlineKeyboardButton("Cᴏɴᴛᴀᴄᴛ", url="telegram.me/J_shree_ram")
                  ]]
            await client.edit_message_media(
                query.message.chat.id, 
                query.message.id, 
...
async def auto_filter(client, msg: Message, edit_message=None, spoll=False, spell_chok=True, **kwargs):
    print(kwargs)
    curr_time = datetime.now(pytz.timezone('Asia/Kolkata')).time()
    ...
    # else:
    if edit_message:
        fuk = await edit_message.edit_text(text=cap, reply_markup=InlineKeyboardMarkup(btn), disable_web_page_preview=True)        
        try:
            if settings['auto_delete']:
                await asyncio.sleep(300)
                await fuk.delete()
    ...
    # Reply with the message indicating lack of delete permission
        ...
        fuk = await m.edit_text(text=cap, reply_markup=InlineKeyboardMarkup(btn), disable_web_page_preview=True)        
        try:
            if settings['auto_delete']:
                await asyncio.sleep(300)
                await fuk.delete()
